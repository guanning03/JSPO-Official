...
def run_ppo(config) -> None:
    ...
    if not ray.is_initialized():
        # 基于缺省 PPO_RAY_RUNTIME_ENV 追加确定性相关 env
        base_env = copy.deepcopy(PPO_RAY_RUNTIME_ENV)
        env = base_env.setdefault("env", {})
        env.update({
            # CUDA/通信/随机性
    ...
    # 给 TaskRunner 同步传入 env（防止某些部署下子进程未继承）
    common_env = {
    ...
    if config.trainer.get("profile_steps") is not None and len(config.trainer.get("profile_steps", [])) > 0:
        ...
        runner = TaskRunner.options(runtime_env={"nsight": nsight_options, "env": common_env}).remote()
    else:
        runner = TaskRunner.options(runtime_env={"env": common_env}).remote()
