...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
    ...
    elif adv_estimator in [AdvantageEstimator.RLOO]:
        ...
        for key, value in pass_at_k_objectives.items():
            data.batch[key] = value
    elif adv_estimator in [AdvantageEstimator.JSPO, AdvantageEstimator.JSPO_PLUS_PLUS]:
        # handle all other adv estimator type other than GAE and GRPO
        adv_estimator_fn = core_algos.get_adv_estimator_fn(adv_estimator)
        adv_kwargs = {
            "token_level_rewards": data.batch["token_level_rewards"],
            "response_mask": data.batch["response_mask"],
            "config": config,
            "js_smooth_coef": global_config.algorithm.js_smooth_coef
        }
        if "uid" in data.non_tensor_batch.keys():  # optional
            adv_kwargs["index"] = data.non_tensor_batch["uid"]
        if "reward_baselines" in data.batch.keys():  # optional
            adv_kwargs["reward_baselines"] = data.batch["reward_baselines"]
        # calculate advantage estimator
        advantages, returns, js_lambda, v_square, s_square, pass_at_k_objectives = adv_estimator_fn(**adv_kwargs)
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns        
        data.batch["js_lambda"] = js_lambda
        data.batch["v_square"] = v_square
        data.batch["s_square"] = s_square
        for key, value in pass_at_k_objectives.items():
            data.batch[key] = value
    else:
        # handle all other adv estimator type other than GAE and GRPO
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        if self.config.algorithm.adv_estimator == AdvantageEstimator.GAE:
            ...
            AdvantageEstimator.JSPO,
            AdvantageEstimator.JSPO_PLUS_PLUS,
        ]:
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # (moved) Balance batch is now executed after advantage computation
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if self.config.algorithm.adv_estimator in [AdvantageEstimator.RLOO, 
                                                                   AdvantageEstimator.GRPO,
                                                                   AdvantageEstimator.JSPO,
                                                                   AdvantageEstimator.JSPO_PLUS_PLUS]:
                            for key in batch.batch.keys():
                                if key.startswith("pass@"):
                                    metrics[f"objectives(train)/{key}"] = torch.mean(batch.batch[key]).item()
                        
                        if self.config.algorithm.adv_estimator in [AdvantageEstimator.JSPO, AdvantageEstimator.JSPO_PLUS_PLUS]:
                            metrics["trace/v_square"] = torch.mean(batch.batch["v_square"]).item()
                            metrics["trace/s_square"] = torch.mean(batch.batch["s_square"]).item()
                            metrics["trace/js_lambda"] = torch.mean(batch.batch["js_lambda"]).item()

                    # Balance the number of valid tokens across DP ranks after advantage computation
                    # to avoid impacting algorithms that assume grouped ordering before this point.
