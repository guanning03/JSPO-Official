...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, num_lora_adapters = 0, **kwargs):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        kwargs["n"] = 1 # NOTE: now hardcode to 1, because we always repeat before rollout

        lora_requests = None
        if self.lora_kwargs:
            lora_int_ids = sorted(list(self.inference_engine.llm_engine.list_loras()))
            if len(lora_int_ids) > 0:
                lora_int_id = lora_int_ids[0]
                # print(f"Using lora id {lora_int_id} during rollout")
                lora_requests = [
