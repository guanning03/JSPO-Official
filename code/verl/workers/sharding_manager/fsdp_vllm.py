...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __init__(
        ...
        self.num_lora_adapters = num_lora_adapters
        ...
        if self.device_mesh is not None:
            ...
            get_torch_device().manual_seed(gen_dp_rank + rollout_seed)  # make sure all tp ranks have the same random states
    ...
    def __enter__(self):
        def __collect_lora_params(adapter_name: str = 'LORA_1') -> OrderedDict:
            """
        ...
        with simple_timer("reshard", self.timing):
            if self.num_lora_adapters == 0:
                ...
                if hasattr(peft_model, "peft_config"):
                    peft_config = peft_model.peft_config.get("LORA_1", None)
                ...
                if self.rollout_config.free_cache_engine:
                    if "tags" in inspect.signature(self.inference_engine.wake_up).parameters:
                        ...
                        self.inference_engine.wake_up()
                for i in range(1, self.num_lora_adapters + 1):
                    # print(f"processing LORA_{i} when global lora id is {FSDPVLLMShardingManager.latest_lora_idx}")
                    get_torch_device().empty_cache()
                    ...
                    peft_model = getattr(self.module, "_fsdp_wrapped_module", self.module)
                    if hasattr(peft_model, "peft_config"):
                        peft_config = peft_model.peft_config.get(name, None)
                        params = __collect_lora_params(name)
                    else:
                        params = self.module.state_dict()
                    params = convert_weight_keys(params, getattr(self.module, "_fsdp_wrapped_module", self.module))
                    log_gpu_memory_usage("After state_dict() in sharding manager memory", logger=logger)
                    ...
                    # update model params
                    self.update_params(params, peft_config=peft_config, adapter_name = name)
                    log_gpu_memory_usage("After sync model weights in sharding manager", logger=logger)
                    del params
    ...
    def update_params(self, updated_params, peft_config=None, **kwargs):
        model = self.model_runner.model
        if peft_config:
            if self.base_sync_done:
                FSDPVLLMShardingManager.latest_lora_idx += 1
                lora_int_id = FSDPVLLMShardingManager.latest_lora_idx
                ...
                # print(f"created lora request number {lora_int_id} for {kwargs['adapter_name']}")
